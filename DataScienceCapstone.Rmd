...
```{r movieeffects, echo=FALSE}
movie_counts = edx %>% group_by(movieIds) %>% summarise(Movies_Rated=n(), Mean_Rating=mean(rating), Standard_Deviation = sd(rating))
movie_counts[1:100,] %>% 
  ggplot(aes(x=userId, y=Movies_Rated)) + 
  geom_bar(stat="identity") + 
...
Movie ratings may be impacted by the genre of the movies. However, each movie may belong to multiple genres. To see if there was a correlation between the genres, I did a Pearson-r correlation with a significance test on the genre matrix. The significance of  correlation is visualized in Figure 5.

```{r genreeffects, echo=FALSE}
genre_matrix = edx[,c(6:23)]
cor_t = rcorr(as.matrix(genre_matrix), type="pearson")
cor_t_p = cor_t$P
corrplot(cor_t_p, method = "color", type = "upper")
```
Figure 5: Significance of correlation between the genres

Correlations between XXX were significant.
...
Given the correlation of genres with ratings, movie ratings will also be impacted by the movie's genres. Table 3 shows the mean rating and standard deviation for each genre. Plotting significance of correlation between ratings and genres, we see no significant interactions (Figure 6). This suggests that individual genres do not have an impact on ratings.
```{r genreeffects, echo=FALSE}
genre_matrix = edx[,c(3,6:23)]
cor_t = rcorr(as.matrix(genre_matrix), type="pearson")
corrplot(cor_t$p, method = "color", type = "upper")
```
...
```{r genrecombo, echo=FALSE}
edx = edx %>% 
  left_join(genre_matrix, by=c('userId', 'movieId'))

genre_corplot = edx[,c(3,26)]
...
```{r genresig, echo=FALSE}
rm(cor_t, cor_t_p, genre_corplot, genre_matrix)
```
...
rm(rating_datetime_counts, rating_year_distance, year_counts)
```
##Results
...
combo_error <- RMSE(pred_combo,validation$rating)

rm(movie_avgs, user_avgs, genre_avgs, pred_combo)
```

The resulting model had an RMSE of `r combo_error`.
...
###Regressions with Regulatization
...
```{r rmse echo=FALSE}
RMSE_table = data.frame(model = c('Movie Bias', 'User Bias', 'Genre Bias', 'Time Bias', 'Year Bias', 'Standardization', 'Regularization'), RMSE = c(movie_error, user_error, genre_error, time_error, year_error, standardization, regularization))

```
Table 6: RMSEs across all models

Based on the theoretical applicability of the knn model, the best regression model is
