...
edx[order(edx$distance_from_release),]
...
pred_umyt_combo_rmses = sapply(lambda_reg,function(l){
  ...
  user_avgs <- edx %>% 
    left_join(movie_avgs, by='movieId') %>%
    group_by(userId) %>% 
    summarise(b_u = sum(rating - mu - b_i, na.rm = TRUE)/n()+l)
  ...
  return(RMSE(pred_umyt_combo,validation$rating))
})
...
The best RMSE was `r r_error` at a lambda value of `r r_error_lambda`. Therefore, regularization did no better than the combined user-movie-distance from release combined model's error.
